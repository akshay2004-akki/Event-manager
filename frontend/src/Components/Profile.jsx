    ...
    <div className="max-w-7xl mx-auto px-4 md:px-8 pt-24">
      ...
      <div className="bg-indigo-600 text-white rounded-t-xl p-6 md:p-10 shadow-lg relative overflow-hidden">
        <div className="absolute top-4 right-4 z- flex gap-3">
          <button
            onClick={navigate}
            className="bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white font-medium py-2 px-4 rounded-lg text-xs inline-flex items-center transition duration-200 shadow-sm hover:shadow-md"
          >
          ...
          <button
            ...
            className="bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white font-medium py-2 px-4 rounded-lg text-xs inline-flex items-center transition duration-200 shadow-sm hover:shadow-md"
          >
            <Pencil size={12} className="mr-1.5" /> Complete Profile
          </button>
          <button
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start sm:space-x-6">
          <div className="relative mb-4 sm:mb-0 flex-shrink-0">
            <p className="w-28 h-28 sm:w-32 sm:h-32 rounded-full flex items-center justify-center border-4 bg-indigo-100 border-indigo-300 shadow-lg object-cover text-5xl text-black">
              {userId?.fullName?.slice(0, 2) || profileData.fullName?.slice(0,2)}
            </p>
          ...
          <div className="text-center sm:text-left flex-1">
            <h1 className="text-2xl sm:text-3xl font-bold drop-shadow-lg">
              {userId?.fullName || profileData.fullName}
            </h1>
            <p className="text-indigo-200 text-sm mt-1">
              <Mail size={14} className="inline mr-1.5" /> {userId?.email || profileData.email}
            </p>
            <p className="text-indigo-200 text-sm">
              <Phone size={14} className="inline mr-1.5" /> {phoneNumber}
            </p>
            <p className="text-indigo-200 text-sm">
            <Badge size={14} className="inline mr-1.5" /> {profileData.rollNumber || ""}
            </p>
      ...
      <div className="bg-white rounded-b-xl shadow-lg p-4 md:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="space-y-6">
          <div>
            <h2 className="text-lg font-semibold text-gray-800 mb-4 border-l-4 border-indigo-500 pl-3">
              Academic Details
            </h2>
            <div className="space-y-4">
              {academicDetails.map((detail) => (
                <div
                  key={detail.label}
                  className={`flex items-center group ${detail.delay}`}
                >
                  <div
                    className={`icon-bg mr-4 transition-colors duration-200 ${detail.color}`}
                  >
                    <detail.icon size={18} />
                  </div>
                  <div>
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-700 mb-3 border-l-4 border-purple-500 pl-3">
              Bio
            </h3>
        ...
        <div className="lg:col-span-2">
          <h2 className="text-lg font-semibold text-gray-800 mb-4 border-l-4 border-sky-500 pl-3">
            Registered Events
          </h2>
            ...
            {registeredEvents.map((event, index) => (
              ...
              >
                ...
                <div className="text-sm text-gray-500 space-y-1">
                  ...
                  <div>
                    <MapPin size={14} className="inline mr-1" /> {event.location}
                  </div>
                  <div>
                    <Clock size={14} className="inline mr-1" /> {event.time}
                  </div>
                  <button
